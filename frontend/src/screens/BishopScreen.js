...
const Bishop = () => {
  ...
  const checkoutHandler = (teacher) => {
    dispatch(setLessonInfo({ price: 400, teacherInfo: teacher }));
    ...
    <>
      ...
      {teachers && teachers.map((teacher, index) => (
        ...
        <Col md={6} className='my-5 text-center'>
          ...
          <h4><em>{teacher && teacher.remainingPeopleForBishop} kişilik yer kaldı <FaExclamationCircle className='warning-icons'/> </em></h4>
        </Col>
      </Row>
      ))}
    </>
